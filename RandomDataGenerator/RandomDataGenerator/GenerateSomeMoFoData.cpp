...
int INITIAL_ID = 0;
long long int DATA_COUNT = 166667;
//int DATA_COUNT = 10;
int ITERATIONS = 1;
...
int writeFile2()
{
	...
	string queryString;
	string filename;
	//long long int startingTime = 2301370304;
	long long int startingTime = 1556409600;
	for (long long int j = 0; j < ITERATIONS; j++) {			//Each iteration is a new file
		filename = "data" + to_string(j) + ".txt";
		myfile.open(filename);
		myfile << HEADER;

		aTime = startingTime;
		cout << aTime << " " << startingTime << endl;
		for (long long int i = 0; i < DATA_COUNT; i++) {		//numberOfData pieces, one per line
			//cout << i << "\n";
			long long int hour = i % 24;
		...
		cout << "Done cars" << endl;
		aTime = startingTime;
		for (long long int i = 0; i < DATA_COUNT; i++) {
			long long int hour = i % 24;
			int coches = getCoches(hour);
